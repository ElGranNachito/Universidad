...
#define DIRECT_BLOCK_COUNT 15-2
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
...

	{
		...
		read_block(inode->block[14], (unsigned char*)&block_buffer);
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	Ext2FSDirEntry* dir_entry_itr = NULL;
	...
	{
		unsigned char block_buffer[BLOCK_SIZE*2];

		auto read_next_block = [&](){
			current_block_offset -= BLOCK_SIZE * (current_block_offset / BLOCK_SIZE);

			read_block(get_block_address(from, block_idx), block_buffer);
			read_block(get_block_address(from, block_idx+1), block_buffer);
		...
		dir_entry_itr = (Ext2FSDirEntry*)(block_buffer + current_block_offset);

		while(strncmp(dir_entry_itr->name, filename, dir_entry_itr->name_length)){
			current_block_offset += dir_entry_itr->record_length;

			if(current_block_offset >= BLOCK_SIZE)
			{
				if(block_idx >= BLOCKS_PER_INODE - 1)
					break;
